...
df_main = pd.DataFrame(columns=['Tweet_ID', 'Username', 'Text', 'Retweets', 'Likes', 'Timestamp'])
...
def get_tweets_from_github(url, df=None, count=None):
    ...
    if df is None:
        df_local = pd.DataFrame(columns=['Tweet_ID', 'Username', 'Text', 'Retweets', 'Likes', 'Timestamp'])
        try:
            response = requests.get(url)
            response.raise_for_status()
        except requests.exceptions.RequestException as e:
            print(f"Error: {e}")
            return df_local  # Return the original DataFrame if there's an error
    ...
    # Extract and format required columns
        df_local['Tweet_ID'] = data['Tweet_ID']
        df_local['Username'] = data['Username']
        df_local['Text'] = data['Text']
        df_local['Retweets'] = data['Retweets']
        df_local['Likes'] = data['Likes']
    # df['Timestamp'] = pd.to_datetime(data['Timestamp'], format='%d-%m-%Y %H:%M') # Assuming 'Timestamp' is in datetime format
        ...
        df_local['Timestamp'] = df_local['Timestamp'].combine_first(
        pd.to_datetime(df['Timestamp'], format=format2, errors='coerce')
    )

    # Save DataFrame to CSV (optional)
        df_local.to_csv("TweetDataset1.csv", index=False)
...
def main():
    ...
    df_main = get_tweets_from_github(github_raw_url, df_main, count=None)
